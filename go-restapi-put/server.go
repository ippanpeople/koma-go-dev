...
func updateBook(w http.ResponseWriter, r *http.Request) {
	...
	for index, item := range books {
		if item.ID == params["id"] {
			// 把新書替換掉舊書
			newBook.ID = item.ID               // 新書id = 老書id
			books[index] = newBook             // 使新書覆蓋老書, 使新書內容更新到結構體裡
			json.NewEncoder(w).Encode(newBook) // 把新書編碼並響應返回給調用端
			return
		}
	}

	// 若用戶指定的id並不在數組中的話, 就只返回使用者更新的書籍id
	json.NewEncoder(w).Encode(ResponseResult{
		Result: "updateBook:" + params["id"],
	})
}
